...
public class OperaController {
	...
	public String newOpera(@ModelAttribute("opera") Opera opera,@RequestParam("artist")String artista,
			@RequestParam("collection")String collezione,Model model, BindingResult bindingResult) {
		this.operaValidator.validate(opera, bindingResult);

		if(!bindingResult.hasErrors()) {
			if(this.artistaService.artistaPerCognome(artista).size()!=0)
				opera.setAutore(this.artistaService.artistaPerCognome(artista).get(0));
